...
{
	BaseType_t res = xTaskCreate( vBasicWEBServer, ( const char * ) "WEB  ", configMINIMAL_STACK_SIZE*2, NULL, 2,  NULL );
	res = 0;
}
	...
	if(err == ERR_OK)
	{
		 ...
		 && ( !strncmp( pcRxString, "GET", 3 ) ))
		{
			...
			if(cDynamicPage)
			{
				...
				sprintf(buf, "ƒ⁄¥Ê”‡¡ø:%d<br>\0", MyGetFreeHeapSize());
...
/*				strcat( cDynamicPage, "<p><pre>Task            Abs Time   % Time<br>************************************************<br>" );
				vTaskGetRunTimeStats(cDynamicPage + strlen( cDynamicPage ) );
*/			
